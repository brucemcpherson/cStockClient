var DomUtils = (function(ns) {
  ...
  */
  ...
  /**
  * disable/enable an array of elements
  * @param {[element]} element
  * @param {boolean} disabled
  * @return {self} 
  */
  ns.setDisabled = function (element, disabled) {
    if (!Array.isArray (element)) element = [element];
    element.forEach (function (d) {
      console.log (d);
      ns.elem (d).disabled = disabled;
    });
    return ns;
  };
  
  ns.disable = function (element) {
    return ns.setDisabled (element , true);
  };
  ns.enable = function (element) {
    return ns.setDisabled (element , false);
  };
  /**
